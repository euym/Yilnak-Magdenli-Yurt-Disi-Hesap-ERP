...
function MiniAllowanceTable({ title, personName, rows }) {
  return <section className="miniAllowanceBox">
    <h3>{title}</h3>
    <p className="hint">{personName || '-'}</p>
    <div className="tableWrap">
      <table>
        <tbody>
          {rows.map(row => <tr key={row.label}><td>{row.label}</td><td><b>{row.value}</b></td></tr>)}
        </tbody>
      </table>
    </div>
  </section>;
...
function ExpenseScreen({ defs, trips, expenses, expense, setExpense, request, reload }) {
  ...
  const safeDefs = defs || {};
  const expenseDefinitions = safeDefs.expenseDefinitions || [];
  const countries = safeDefs.countries || [];
  const safeTrips = trips || [];
  const safeExpenses = expenses || [];
  const currentExpense = expense || blankExpense;

  const selectedDef = expenseDefinitions.find(x => x.id === currentExpense.expense_definition_id) || null;
  const isFuel = selectedDef?.category === 'Yakıt';
  const needsFuelStatus = isFuel && currentExpense.vehicle_type === 'Çekici';
  const needsLiter = isFuel;

  const selectedTripExpenses = currentExpense.trip_id
    ? safeExpenses.filter(item => item.trip_id === currentExpense.trip_id)
    : safeExpenses;
  ...
  const selectedTrip = safeTrips.find(t => t.id === currentExpense.trip_id) || null;
  const activeAllowanceDef = (safeDefs.allowanceDefinitions || []).find(x => x.is_active) || (safeDefs.allowanceDefinitions || [])[0] || null;
  const allowanceDaily = {
    domestic: activeAllowanceDef?.domestic_daily_amount || 0,
    abroad: activeAllowanceDef?.abroad_daily_amount || 0
  };
  ...
  const tractorDays = calcAllowanceDaysFromDates(tractorDates);
  ...
  const tractorDomesticTotal = tractorDays.domesticDays * numberValue(allowanceDaily.domestic);
  const tractorAbroadTotal = tractorDays.abroadDays * numberValue(allowanceDaily.abroad);
  const escortDomesticTotal = escortDays.domesticDays * numberValue(allowanceDaily.domestic);
  const escortAbroadTotal = escortDays.abroadDays * numberValue(allowanceDaily.abroad);

  function tripOptionLabel(t) {
    const d = t?.created_at ? new Date(t.created_at) : null;
    const dateText = d && !Number.isNaN(d.getTime()) ? d.toLocaleDateString('tr-TR') : '';
    return `${t?.project_name || 'Projesiz'}${dateText ? ' - ' + dateText : ''}`;
  }

  function setExpenseField(name, value) {
    setExpense(prev => {
      const next = { ...(prev || blankExpense), [name]: value };
      if (name === 'expense_definition_id') {
        const def = expenseDefinitions.find(x => x.id === value);
  ...
  async function saveExpense(e) {
    e.preventDefault();
    try {
      if (!currentExpense.trip_id) return alert('Sefer seçiniz.');
      if (!currentExpense.expense_definition_id) return alert('Masraf türü seçiniz.');
      if (!currentExpense.amount) return alert('Tutar giriniz.');
      if (needsFuelStatus && !currentExpense.fuel_status) return alert('Çekici yakıtı için Boş/Dolu seçiniz.');
      if (needsLiter && !currentExpense.liter) return alert('Yakıt için litre giriniz.');

      const payload = cleanEmptyValues({
        ...currentExpense,
        city_id: null,
        fuel_status: needsFuelStatus ? currentExpense.fuel_status : ''
      });
      ...
      setExpense({ ...blankExpense, trip_id: currentExpense.trip_id });
      await reload();
  ...
  return <div className="layout">
    ...
    <main className="card"><h2>{editingExpenseId ? 'Masraf Düzeltme' : 'Masraf Girişi'}</h2>
      <form onSubmit={saveExpense}>
        <div className="grid two">
          <Select label="Sefer seç" value={currentExpense.trip_id} onChange={v => setExpenseField('trip_id', v)} options={safeTrips.map(t => ({ ...t, label: tripOptionLabel(t) }))} textKey="label" />
          <Select label="Ülke" value={currentExpense.country_id} onChange={v => setExpenseField('country_id', v)} options={countries} textKey="name" />
          <Select label="Masraf türü" value={currentExpense.expense_definition_id} onChange={v => setExpenseField('expense_definition_id', v)} options={expenseDefinitions} textKey="name" />
          <ReadOnly label="Kategori" value={selectedDef?.category || '-'} />
          <select value={currentExpense.vehicle_type || ''} onChange={e => setExpenseField('vehicle_type', e.target.value)}><option value="">Araç tipi</option><option>Çekici</option><option>Öncü</option><option>Diğer</option></select>
        </div>

        {isFuel && <div className="fuelBox">
          {needsFuelStatus && <>
            <b>Yakıt Durumu</b>
            <label><input type="radio" name="fuel_status" checked={currentExpense.fuel_status === 'Boş'} onChange={() => setExpenseField('fuel_status', 'Boş')} /> Boş</label>
            <label><input type="radio" name="fuel_status" checked={currentExpense.fuel_status === 'Dolu'} onChange={() => setExpenseField('fuel_status', 'Dolu')} /> Dolu</label>
          </>}
          {!needsFuelStatus && <b>Yakıt Durumu: Öncü / Diğer araç için Boş-Dolu seçilmez</b>}
          <input type="number" step="0.01" placeholder="Litre" value={currentExpense.liter || ''} onChange={e => setExpenseField('liter', e.target.value)} />
        </div>}

        <div className="grid two expenseBottom">
          <input required type="number" step="0.01" placeholder="Tutar" value={currentExpense.amount || ''} onChange={e => setExpenseField('amount', e.target.value)} />
          <select value={currentExpense.currency || selectedDef?.default_currency || 'TRY'} onChange={e => setExpenseField('currency', e.target.value)}><option>TRY</option><option>EUR</option><option>USD</option></select>
          <input type="date" value={currentExpense.expense_date || ''} onChange={e => setExpenseField('expense_date', e.target.value)} />
          <input placeholder="Açıklama" value={currentExpense.note || ''} onChange={e => setExpenseField('note', e.target.value)} />
        </div>
        <div className="formActions">
      ...
      <h3 className="summaryMainTitle">Harcırah Tabloları</h3>
      <div className="cleanSummaryGrid">
        <MiniAllowanceTable title="Çekici Yurt İçi Harcırah" personName={tractorDriverName} rows={[
          { label: 'Yurt İçi Sefer Başlangıç', value: tractorDates.domestic_start_date || '-' },
          { label: 'Yurt İçi Çıkış', value: tractorDates.domestic_exit_date || '-' },
          { label: 'Yurt İçi Giriş', value: tractorDates.domestic_return_date || '-' },
          { label: 'Yurt İçi Sefer Bitiş', value: tractorDates.domestic_end_date || '-' },
          { label: 'Yurt İçi Geçen Gün', value: tractorDays.domesticDays.toLocaleString('tr-TR') },
          { label: 'Günlük Harcırah', value: `${numberValue(allowanceDaily.domestic).toLocaleString('tr-TR')} ${allowanceCurrency.domestic}` },
          { label: 'Yurt İçi Harcırah', value: `${tractorDomesticTotal.toLocaleString('tr-TR')} ${allowanceCurrency.domestic}` }
        ]} />

        <MiniAllowanceTable title="Çekici Yurt Dışı Harcırah" personName={tractorDriverName} rows={[
          { label: 'Yurt Dışı Giriş', value: tractorDates.abroad_entry_date || '-' },
          { label: 'Yurt Dışı Çıkış', value: tractorDates.abroad_exit_date || '-' },
          { label: 'Yurt Dışı Geçen Gün', value: tractorDays.abroadDays.toLocaleString('tr-TR') },
          { label: 'Günlük Harcırah', value: `${numberValue(allowanceDaily.abroad).toLocaleString('tr-TR')} ${allowanceCurrency.abroad}` },
          { label: 'Yurt Dışı Harcırah', value: `${tractorAbroadTotal.toLocaleString('tr-TR')} ${allowanceCurrency.abroad}` }
        ]} />

        <MiniAllowanceTable title="Öncü Yurt İçi Harcırah" personName={escortDriverName} rows={[
          { label: 'Yurt İçi Sefer Başlangıç', value: escortDates.domestic_start_date || '-' },
          { label: 'Yurt İçi Çıkış', value: escortDates.domestic_exit_date || '-' },
          { label: 'Yurt İçi Giriş', value: escortDates.domestic_return_date || '-' },
          { label: 'Yurt İçi Sefer Bitiş', value: escortDates.domestic_end_date || '-' },
          { label: 'Yurt İçi Geçen Gün', value: escortDays.domesticDays.toLocaleString('tr-TR') },
          { label: 'Günlük Harcırah', value: `${numberValue(allowanceDaily.domestic).toLocaleString('tr-TR')} ${allowanceCurrency.domestic}` },
          { label: 'Yurt İçi Harcırah', value: `${escortDomesticTotal.toLocaleString('tr-TR')} ${allowanceCurrency.domestic}` }
        ]} />

        <MiniAllowanceTable title="Öncü Yurt Dışı Harcırah" personName={escortDriverName} rows={[
          { label: 'Yurt Dışı Giriş', value: escortGoesAbroad ? (escortDates.abroad_entry_date || '-') : '' },
          { label: 'Yurt Dışı Çıkış', value: escortGoesAbroad ? (escortDates.abroad_exit_date || '-') : '' },
          { label: 'Yurt Dışı Geçen Gün', value: escortGoesAbroad ? escortDays.abroadDays.toLocaleString('tr-TR') : '' },
          { label: 'Günlük Harcırah', value: escortGoesAbroad ? `${numberValue(allowanceDaily.abroad).toLocaleString('tr-TR')} ${allowanceCurrency.abroad}` : '' },
          { label: 'Yurt Dışı Harcırah', value: escortGoesAbroad ? `${escortAbroadTotal.toLocaleString('tr-TR')} ${allowanceCurrency.abroad}` : '' }
        ]} />
      </div>

      <h3>Girilen Masraflar</h3><div className="tableWrap"><table><thead><tr><th>Masraf</th><th>Kategori</th><th>Ülke</th><th>Araç</th><th>Durum</th><th>Litre</th><th>Tutar</th><th>Para</th><th>Tarih</th><th>İşlem</th></tr></thead><tbody>{selectedTripExpenses.length ? selectedTripExpenses.map(x => <tr key={x.id} className={editingExpenseId === x.id ? 'editingRow' : ''}><td>{x.expense_name || '-'}</td><td>{x.category || '-'}</td><td>{x.country_name || '-'}</td><td>{x.vehicle_type || '-'}</td><td>{x.fuel_status || '-'}</td><td>{x.liter || '-'}</td><td>{x.amount || '-'}</td><td>{x.currency || '-'}</td><td>{x.expense_date || '-'}</td><td className="rowActions"><button type="button" onClick={() => editExpense(x)}>Düzelt</button><button type="button" className="danger" onClick={() => deleteExpense(x)}>Sil</button></td></tr>) : <tr><td colSpan="10">{currentExpense.trip_id ? 'Seçili sefere ait masraf yok.' : 'Henüz masraf girilmedi.'}</td></tr>}</tbody></table></div>
    </main>
  </div>;
}
...
function ExpenseSummaryScreen({ defs, trips, expenses }) {
  ...
  const [mergedOtherRows, setMergedOtherRows] = useState([]);
  const selectedTrip = trips.find(t => t.id === selectedTripId) || null;
  const tripExpenses = selectedTripId ? expenses.filter(x => x.trip_id === selectedTripId) : [];
  ...
  function buildMergedOtherTable() {
    if (!selectedOtherItems.length) {
      setMergedOtherRows([]);
      alert('Birleştirmek için en az bir diğer masraf seçiniz.');
      return;
    }
    ...
    selectedOtherItems.forEach(item => {
      ...
      const key = `${item.expense_name || 'Diğer Masraf'}|${item.category || 'Diğer'}|${currency}`;
      if (!groups[key]) {
        groups[key] = {
          name: item.expense_name || 'Diğer Masraf',
      ...
      groups[key].amount += numberValue(item.amount);
      groups[key].count += 1;
      groups[key].details.push(item.note || item.country_name || item.expense_date || '-');
    });

    setMergedOtherRows(Object.values(groups));
  }
    ...
    <main className="card">
      ...
      <div className="cleanSummaryGrid">
        ...
        <section className="expenseSummaryBox wide">
          ...
          <div className="formActions">
            <button className="primary" type="button" onClick={buildMergedOtherTable}>Seçilenleri Birleştir</button>
            <button className="ghost" type="button" onClick={() => { setSelectedOtherIds([]); setMergedOtherRows([]); }}>Seçimi Temizle</button>
          </div>
          ...
          {mergedOtherRows.length > 0 && <div className="tableWrap mergedOtherTable">
            <h4>Birleştirilen Diğer Masraflar Tablosu</h4>
            <table>
              <thead><tr><th>Masraf</th><th>Kategori</th><th>Adet</th><th>Toplam Tutar</th><th>Para</th><th>Detay</th></tr></thead>
              <tbody>{mergedOtherRows.map(row => <tr key={`${row.name}-${row.category}-${row.currency}`}><td>{row.name}</td><td>{row.category}</td><td>{row.count}</td><td>{row.amount.toLocaleString('tr-TR')}</td><td>{row.currency}</td><td>{row.details.join(' / ')}</td></tr>)}</tbody>
            </table>
          </div>}
        </section>
      </div>
